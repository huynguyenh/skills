...
**Before choosing colors, fonts, or visual styling for any ZenLabs project diagram**, load `/hnh-design-guideline` to get the brand palette — it includes an Excalidraw-specific section. This applies to architecture diagrams, system diagrams, flowcharts, and any visual output tied to a ZenLabs project.

### When to apply branding
- The diagram is for a ZenLabs project (GMD, ENAT, Golden, etc.)
- The user mentions "branded", "our colors", "professional", or "ZenLabs style"
- The diagram will be shared externally (clients, presentations, docs)

### When to skip branding
- Quick whiteboard sketches or internal brainstorming
- The user explicitly asks for default/generic styling
- Non-ZenLabs projects

### ZenLabs Diagram Style (Default for All ZenLabs Projects)

This is the canonical style. Follow it unless the user explicitly requests something different.

**Color Palette — Monochrome + Green Accent:**

| Element | Color | Hex |
|---|---|---|
| Canvas background | White | `#FFFFFF` |
| Service box fill | White (no fill) | `#FFFFFF` |
| Service box stroke | Black | `#000000` |
| Zone container stroke | Dark teal/green | `#1e7a5e` |
| Zone label text | Dark teal/green | `#1e7a5e` |
| Namespace container stroke | Light green | `#43CE81` |
| Namespace label text | Light green | `#43CE81` |
| Service label text | Black | `#000000` |
| Arrows | Black | `#000000` |
| Severity: OK | Green | `#179F65` |
| Severity: Warning | Orange | `#EB7E11` |
| Severity: Critical | Red | `#F03333` |

**Component Styles:**

| Component | Shape | Stroke | Fill | Corner |
|---|---|---|---|---|
| Zone containers | Rectangle | Dashed, ~2px, `#1e7a5e` | None (transparent) | Rounded |
| Namespace groups | Rectangle | Dashed, ~2px, `#43CE81` | None (transparent) | Rounded |
| Service boxes | Rectangle | Solid, ~2-3px, `#000000` | White | Rounded |
| Placeholder boxes | Rectangle | Dashed, ~2px, `#000000` | None | Rounded |
| Arrows/connectors | Arrow | Solid, ~2px, `#000000` | N/A | N/A |

**Typography — Hand-drawn (Virgil) font throughout:**

| Element | Font Size | Notes |
|---|---|---|
| Title | ~28px | Top of diagram |
| Zone labels | ~20px | "Route53 DNS", "Virtual Private Cloud", etc. |
| Namespace labels | ~18px | "namespace: backend", etc. |
| Service box text | ~16-18px | "gmd-tms-backend", "NLB", etc. |
| Secondary labels | ~14px | "Users", annotations |
...
| Main flow | Center-aligned |
| Sidebar groups | Left-aligned, stacked vertically |
| Nesting depth | Up to 3 levels (VPC → EKS → Namespace → Service) |
| Vertical spacing | 40-60px between service boxes |
| Horizontal gaps | 30-40px between sibling boxes |
| Zone padding | 30-40px from border to inner content |
| Dashed = grouping | Zones and namespaces use dashed stroke |
| Solid = components | Service boxes use solid stroke |
| No fills | All boxes are white/transparent — hierarchy via stroke style only |

**Key Principles:**
1. Monochrome + one accent color (green) — no rainbow
2. Hand-drawn (Virgil) font everywhere — no formal sans-serif
3. Dashed for grouping, solid for components — clear visual hierarchy
4. No background fills — hierarchy communicated through stroke style and nesting
5. Clean separation — sidebars (Monitoring, Gitops) visually separated from main flow
...
1b. **If ZenLabs project:** Load `/hnh-design-guideline` and follow the ZenLabs Diagram Style section above (monochrome + green accent, dashed zones, solid services, no fills, Virgil font).
...
1. Plan your coordinate grid first. **If ZenLabs project:** Load `/hnh-design-guideline` and follow the ZenLabs Diagram Style (monochrome + green, dashed zones, solid services, no fills).
